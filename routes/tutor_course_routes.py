...
from .schemas import TutorCourseDto
...
@router.post(path="/users/{user_id}/subjects/{subject_id}/", status_code=status.HTTP_201_CREATED,
             response_model=TutorCourseDto, description="Add course for tutor")
async def add_course(user_id: int, subject_id: int, db: Session = Depends(get_db)):
    db_course = tutor_course_crud.add_course(db=db, user_id=user_id, subject_id=subject_id)
    return db_course
